...
├── lib.rs               # (~2750行) 核心业务逻辑，所有 Tauri commands，全局状态管理
├── git_ops.rs           # (~400行) Git 操作（git2 读取 + Command 写入）
├── pty_manager.rs       # (~270行) PTY 伪终端管理（shell 会话生命周期）
└── http_server.rs       # (~1000行) Axum HTTP/WS 服务器（浏览器模式）
...
├── hooks/
│   ├── useWorkspace.ts  # (~310行) 工作区状态管理（CRUD、Git操作、锁定）
│   ├── useTerminal.ts   # (~360行) 终端状态与多窗口同步（广播、防抖、去重）
│   └── useUpdater.ts    # 应用更新器状态
└── lib/
    ├── backend.ts       # (~337行) API 适配层（Tauri IPC / HTTP 自动切换）
...
- **Git (10)**: switch_branch, clone_project, sync_base, push, merge_test/base, diff_stats, create_pr, check/get_branches
