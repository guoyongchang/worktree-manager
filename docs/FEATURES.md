...
#### 5.5 后台远程同步 (Background Remote Sync)
- **功能**：后台获取远程仓库最新信息
- **两阶段加载**：
  1. Phase 1: 显示本地数据（毫秒级响应）
  2. Phase 2: 后台执行 `git fetch origin`，完成后刷新分支状态
- **进度指示**：同步期间显示进度条，禁用操作按钮
- **性能优化**：`check_remote_branch_exists` 使用本地 `git branch -r` 查询，不触发网络请求
...
#### 6.6 按工作区隔离
- **状态保存**：切换 worktree 时自动保存当前终端标签状态
- **状态恢复**：切回时自动恢复之前的终端标签和活动标签
- **PTY 生命周期**：切走时 PTY 会话关闭，切回时自动重建
