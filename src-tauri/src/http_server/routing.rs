...
use super::{
    ...
    h_get_main_occupation, h_get_main_workspace_status, h_get_mirror_sources, h_get_ngrok_token,
    h_get_opened_workspaces, h_get_remote_branches, h_get_share_info, h_get_share_state,
    h_get_shell_integration_enabled, h_get_skip_git_hooks, h_get_terminal_state,
    h_get_voice_asr_model, h_get_voice_refine_base_url, h_get_voice_refine_enabled,
    h_get_voice_refine_model, h_get_workspace_config, h_import_external_project, h_kick_client,
    h_list_dashscope_models, h_list_vault_item_children, h_list_workspaces, h_list_worktrees,
    ...
    h_pty_read, h_pty_resize, h_pty_write, h_pull_current_branch, h_push_to_remote,
    ...
    h_voice_stop, h_ws_upgrade, is_allowed_origin, load_mcp_config, save_mcp_config, McpConfig,
};
...
pub(super) fn build_api_router(cert_pem: Option<String>) -> Router {
    let mut router = Router::new()
        ...
        .route("/api/update_worktree_color", post(h_update_worktree_color))
        .route(
        ...
        .route("/api/pull_current_branch", post(h_pull_current_branch))
        ...
        .route("/api/get_crash_report", get(h_get_crash_report))
        .route("/api/frontend_log", post(h_frontend_log))
        ...
        .route(
            "/api/get_voice_refine_base_url",
            post(h_get_voice_refine_base_url),
        )
        .route(
            "/api/set_voice_refine_base_url",
            post(h_set_voice_refine_base_url),
        )
        .route("/api/get_voice_asr_model", post(h_get_voice_asr_model))
        .route("/api/set_voice_asr_model", post(h_set_voice_asr_model))
        .route(
            "/api/get_voice_refine_model",
            post(h_get_voice_refine_model),
        )
        .route(
            "/api/set_voice_refine_model",
            post(h_set_voice_refine_model),
        )
        .route("/api/list_dashscope_models", post(h_list_dashscope_models))
        .route(
            "/api/check_dashscope_api_key",
            post(h_check_dashscope_api_key),
        )
        // Commit AI key
        .route("/api/get_commit_ai_api_key", post(h_get_commit_ai_api_key))
        ...
        .route(
            "/api/get_commit_prefix_config",
            post(h_get_commit_prefix_config),
        )
        .route(
            "/api/set_commit_prefix_config",
            post(h_set_commit_prefix_config),
        )
        .route(
            "/api/get_git_user_global_config",
            post(h_get_git_user_global_config),
        )
        .route(
        ...
        .route(
            "/api/get_shell_integration_enabled",
            post(h_get_shell_integration_enabled),
        )
        .route(
            "/api/set_shell_integration_enabled",
            post(h_set_shell_integration_enabled),
        )
        ...
        .route(
            "/api/speed_test_single_mirror",
            post(h_speed_test_single_mirror),
        )
        ...
        .route(
            ...
            post(h_list_vault_item_children),
        )
