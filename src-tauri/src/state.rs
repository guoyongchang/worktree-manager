...
use crate::types::{
    ShareState, ConnectedClient, TerminalState, AuthRateLimiter,
    GlobalConfig, WorkspaceConfig,
};

// PTY Manager 全局实例
pub(crate) static PTY_MANAGER: Lazy<Mutex<PtyManager>> = Lazy::new(|| Mutex::new(PtyManager::new()));

// 多窗口 workspace 绑定：window_label -> workspace_path
pub(crate) static WINDOW_WORKSPACES: Lazy<Mutex<HashMap<String, String>>> = Lazy::new(|| Mutex::new(HashMap::new()));

// 多窗口 worktree 锁定：(workspace_path, worktree_name) -> window_label
// 同一 worktree 只能被一个窗口独占选中
pub(crate) static WORKTREE_LOCKS: Lazy<Mutex<HashMap<(String, String), String>>> = Lazy::new(|| Mutex::new(HashMap::new()));

// ==================== 分享状态 ====================

pub(crate) static SHARE_STATE: Lazy<Mutex<ShareState>> = Lazy::new(|| Mutex::new(ShareState::default()));
...
// Increased capacity from 64 to 256 to reduce message lag and drops
pub(crate) static TERMINAL_STATE_BROADCAST: Lazy<tokio::sync::broadcast::Sender<String>> = Lazy::new(|| {
    let (tx, _) = tokio::sync::broadcast::channel(256);
    tx
});
...
pub(crate) static APP_HANDLE: Lazy<Mutex<Option<tauri::AppHandle>>> = Lazy::new(|| Mutex::new(None));
...
pub(crate) static WORKSPACE_CONFIG_CACHE: Lazy<Mutex<Option<(String, WorkspaceConfig)>>> = Lazy::new(|| Mutex::new(None));
