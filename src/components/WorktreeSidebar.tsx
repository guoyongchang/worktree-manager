      ...
      <div className="space-y-0.5">
        ...
        <div className="flex items-center gap-2 min-h-[24px]">
          ...
          <button
            ...
            onClick={onToggleWms}
            disabled={wmsLoading}
            className={`relative inline-flex h-4 w-7 items-center rounded-full shrink-0 transition-colors ${wmsLoading ? 'opacity-50 cursor-wait' : 'cursor-pointer'
              } ${wmsUrl ? 'bg-purple-500' : 'bg-slate-600'}`}
          >
            <span className={`inline-block h-3 w-3 rounded-full bg-white transition-transform ${wmsUrl ? 'translate-x-3.5' : 'translate-x-0.5'
              }`} />
          </button>
        </div>
      </div>
                ...
                <Tooltip key={wt.name}>
                  <TooltipTrigger asChild>
                    <button
                      ...
                      className={`h-8 w-8 flex items-center justify-center rounded-md transition-colors shrink-0 ${isLockedByOther && _isTauri
  ...
  return (
    <>
      ...
      <div className="w-72 bg-slate-800/50 border-r border-slate-700/50 flex flex-col shrink-0 max-sm:fixed max-sm:inset-y-0 max-sm:left-0 max-sm:z-50 max-sm:w-[85vw] max-sm:max-w-[320px] max-sm:bg-slate-800">
        ...
        {mainWorkspace && (
          <div
            className={`px-4 py-3 border-b border-slate-700/50 cursor-pointer transition-all duration-150 border-l-2 ${!selectedWorktree ? "bg-slate-700/30 border-l-blue-500" : "border-l-transparent hover:bg-slate-700/20"
              }`}
            onClick={() => onSelectWorktree(null)}
          >
            <div className="flex items-center justify-between">
        ...
        <div className="flex-1 overflow-y-auto">
          ...
          {activeWorktrees.length === 0 ? (
            ...
            activeWorktrees.map(wt => {
              const lockedBy = lockedWorktrees[wt.name];
              const isLockedByOther = lockedBy && lockedBy !== currentWindowLabel;
              const isDeployed = wt.name === occupation?.worktree_name;
              // 网页端允许选择被锁定的工作区（远程查看模式）
              const canSelect = (!isLockedByOther || !_isTauri) && !isDeployed;
              return (
                <div
                  key={wt.name}
                  ...
                  <div className="flex items-center gap-2.5">
                    ...
                    {isDeployed && (
                      <TooltipProvider delayDuration={300}>
                        <Tooltip>
                          <TooltipTrigger asChild>
                            <span className="text-[10px] text-blue-400/80 bg-blue-900/20 border border-blue-800/30 px-1.5 py-0.5 rounded shrink-0 cursor-help">{t('deploy.deployed')}</span>
                          </TooltipTrigger>
                          <TooltipContent side="right">{t('deploy.deployedTooltip')}</TooltipContent>
                        </Tooltip>
                      </TooltipProvider>
                    )}
                    ...
                    {wt.projects.some(p => p.has_uncommitted) && !isLockedByOther && !isDeployed && (() => {
          ...
          {showArchived && archivedWorktrees.map(wt => (
            <div
              key={wt.name}
              className={`px-4 py-2.5 cursor-pointer transition-colors opacity-60 ${selectedWorktree?.name === wt.name ? "bg-slate-700/30" : "hover:bg-slate-700/20"
                }`}
